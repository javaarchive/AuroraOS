...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var location = parseURL(req.url.slice(6));
    ...
    // Don't care
    if (!hasRequiredHeaders(req.headers) && false) {
    ...
    var proxyBaseUrl =
      (isRequestedOverHttps ? "https://" : "http://") +
      req.headers.host +
      "/cors";
    ...
    if (url.parse(location).hostname != process.env.HOSTINGURL) {
      req.session.lasturl = url.parse(location).hostname;
    }
...
exports.createServer = function createServer(options, curServer) {
  ...
  proxy.on("proxyReq", function(proxyReq, req, res, options2) {
    for (var i = 0; i < options.removeHeaders.length; i++)
      try {
        proxyReq.removeHeader(options.removeHeaders[i]);
      } catch (ex) {}
    //console.log("HOST: "+proxyReq);
    proxyReq.setHeader("Host",req.session.lasturl);
  });
  ...
  curServer.get("/cors/*", requestHandler);
